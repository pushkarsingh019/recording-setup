...
import getTrialData from "../../backend/getTrialData";
...
function HomeScreen() {
    ...
    const [trialStage, setTrialStage] = useState(1);
    const [trialData, setTrialData] = useState({});

    // Fetch trial data on component mount
    useEffect(() => {
    ...
    const startTrial = async () => {
        ...
        await axios.get(`${backendUrl}/stimulus/${Number(trialData.stimulus)}`);
        await axios.get(`${backendUrl}/side/${Number(trialData.side)}`)
        try {
    ...
    switch (trialStage) {
        case 1:
            ...
            <br />
            <div>{trialData !== undefined ? <div>
            <p><strong>{trialData.stimulus === 0 ? "Negative Stimulus" : "Positive Stimulus"}</strong></p>
            <p><strong>{trialData.side === 0 ? "Left Side" : "Right Side"}</strong></p>
            </div> : <p><i>press <strong>t</strong> to get trial information.</i></p>}</div>
            <br />
            <div>
            <button
        ...
        case 2:
            return(
                <div>
                    ...
                    <button className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700" onClick={() => detect(2)}>no detection (m)</button>
                </div>
            )
